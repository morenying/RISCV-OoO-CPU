...
// Requirements: 6.4, 6.5, 1.3, 7.1, 7.2
//=================================================================
...
module csr_unit #(
    ...
    input  wire                interrupt_i,   // Distinguish interrupt from exception
    ...
    output wire [3:0]          irq_code_o,    // Interrupt code for mcause
    ...
    // Priority: External > Software > Timer (RISC-V spec)
    //=========================================================
    wire ext_irq_enabled   = mie_meie && mip_meip;
    wire sw_irq_enabled    = mie_msie && mip_msip;
    wire timer_irq_enabled = mie_mtie && mip_mtip;
    
    assign irq_pending_o = mstatus_mie && (ext_irq_enabled || sw_irq_enabled || timer_irq_enabled);
    
    // Determine highest priority interrupt code
    assign irq_code_o = ext_irq_enabled   ? INT_EXT_M :
                        sw_irq_enabled    ? INT_SW_M :
                        timer_irq_enabled ? INT_TIMER_M :
                        4'd0;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            //=================================================
            // Interrupt Handling (higher priority than exception)
            //=================================================
            if (interrupt_i) begin
                ...
                // Save interrupt info
                mepc <= exc_pc_i;
                mcause_interrupt <= 1'b1;  // This is an interrupt
                mcause_code <= exc_code_i;
                mtval <= 32'h0;  // mtval is 0 for interrupts
            end
            //=================================================
            // Exception Handling
            //=================================================
                ...
                // Save exception info
                mepc <= exc_pc_i;
                mcause_interrupt <= 1'b0;  // This is an exception
                mcause_code <= exc_code_i;
                mtval <= exc_tval_i;
            end
            //=================================================
