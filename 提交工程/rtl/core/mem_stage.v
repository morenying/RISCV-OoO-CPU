...
module mem_stage #(
    ...
    //=========================================================
    // D-Cache Interface (with bus error support)
    //=========================================================
    ...
    input  wire [1:0]              dcache_resp_error_i,  // AXI response: 00=OKAY, 10=SLVERR, 11=DECERR
    ...
    output wire [XLEN-1:0]         lsu_cdb_exc_tval_o,  // Faulting address for bus errors
    ...
    localparam EXC_LOAD_ACCESS    = 4'd5;   // Load access fault (bus error)
    localparam EXC_STORE_MISALIGN = 4'd6;   // Store address misaligned
    localparam EXC_STORE_ACCESS   = 4'd7;   // Store access fault (bus error)
    ...
    reg [XLEN-1:0] result_exc_tval;  // Faulting address
    ...
    assign lsu_cdb_exc_tval_o = result_exc_tval;
    ...
    // Bus Error Detection
    //=========================================================
    wire bus_error = dcache_resp_valid_i && (dcache_resp_error_i != RESP_OKAY);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                IDLE: begin
                    if (lsu_issue_valid_i && lsu_issue_ready_o) begin
                        ...
                        if (addr_misaligned) begin
                            ...
                            result_exc_code <= lsu_issue_is_load_i ? EXC_LOAD_MISALIGN : EXC_STORE_MISALIGN;
                            result_exc_tval <= mem_addr;  // Faulting address
                    ...
                    if (sq_valid[sq_head] && sq_committed[sq_head] && state == IDLE) begin
                        ...
                        pend_is_load <= 1'b0;  // This is a store
                        state <= STORE_REQ;
                    end
                end
                ...
                LOAD_WAIT: begin
                    if (dcache_resp_valid_i) begin
                        ...
                        if (bus_error) begin
                            result_data <= 0;
                            result_exception <= 1'b1;
                            result_exc_code <= EXC_LOAD_ACCESS;
                            result_exc_tval <= pend_addr;  // Faulting address
                        end else begin
                ...
                STORE_WAIT: begin
                    if (dcache_resp_valid_i) begin
                        // Check for bus error on store
                        if (bus_error) begin
                            // Store bus error - this is a serious error
                            // In a real system, this would trigger an exception
                            // For committed stores, we need to handle this specially
                            // For now, we log it and continue (store already committed)
                            // A more complete implementation would use a store error queue
                        end
